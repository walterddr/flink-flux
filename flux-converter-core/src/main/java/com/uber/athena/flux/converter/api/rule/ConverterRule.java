...
public abstract class ConverterRule<IN extends Node, OUT extends Node> implements Rule {
  ...
  private final Class<IN> inTrait;
  private final Class<OUT> outTrait;
   ...
   */
  public abstract void onMatch(IN node, ConverterContext context);
   ...
   */
  public abstract boolean matches(IN node, ConverterContext context);
  ...
  public <T extends Node> boolean matches(T node) {
    Class nodeClass = node.getClass();
    if (inTrait.equals(nodeClass)) {
      return matches((IN) node, context);
    } else {
      throw new UnsupportedOperationException("Illegal node conversion! converter accepts: "
          + inTrait + " while node type: " + nodeClass);
    }
   ...
   */
  public abstract OUT convertNode(IN node, ConverterContext context);
}
