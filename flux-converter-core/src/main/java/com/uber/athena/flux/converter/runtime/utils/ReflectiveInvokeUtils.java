...
package com.uber.athena.flux.converter.runtime.utils;
  ...
  public static Object buildObject(
    ...
    if (def.getConstructorArgs() != null && def.getConstructorArgs().size() > 0) {
      ...
      if (def.getHasReferenceInArgs()) {
        cArgs = VertexConverterUtils.resolveReferences(cArgs, traverserContext, converterContext);
      }
    ...
    applyProperties(def, obj, traverserContext, converterContext);
    invokeConfigMethods(def, obj, traverserContext, converterContext);
    return obj;
  }
  ...
  private static void applyProperties(
    ...
    if (props != null) {
      Map<String, Object> propertyToObjectMap =
          VertexConverterUtils.resolveProperties(props, traverserContext, converterContext);
  ...
  private static void invokeConfigMethods(
    ...
    for (ConfigMethodDef methodDef : methodDefs) {
      ...
      if (methodDef.getHasReferenceInArgs()) {
        args = VertexConverterUtils.resolveReferences(args, traverserContext, converterContext);
      }
